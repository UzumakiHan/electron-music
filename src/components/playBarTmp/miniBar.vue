    ...
    <div class="playbar-mini">
        <div class="wrapper" :class="{ 'active': isMini }">
            ...
            <div class="play-main">
                ...
                <div class="play-singer"><router-link :to="{ path: '/singer', query: { id: author.id } }" class="song_author"
                        v-for="(author, k) in curSongInfo.singer" :key="author.name">{{ k !== 0 ? ' / ' + author.name :
                            author.name }}</router-link></div>
                <div class="play-time">
                    <span>{{ handleFormatSongTime((handleFormatSongSecond(curSongInfo.duration) - currentTime) *
                        1000) }}</span>
                </div>
                <div class="audioProgress">
                    <ProgressLine :progressWidth="audioProgressWidth" :isShowBtn="false"
...
const emits = defineEmits(['audioHandler', 'playAudioMode', 'setAudioProgress', 'changeMini'])
