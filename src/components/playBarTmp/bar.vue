<template>
    <div :class="['play-bar', lockName]" v-if="curSongInfo" @mouseenter="enterBar($event)" @mouseleave="leaveBar($event)">
        ...
        <!-- 播放进度条 -->
        <progress-line class="audioProgress" :progressWidth="audioProgressWidth"
        ...
        <div class="wrapper">
            <div class="play-bar-inside">
                <div class="bar-l">
                    ...
                    <div class="bar-name">
                        <router-link :to="{ path: '/song', query: { id: curSongInfo.id } }"
                            class="song_name">{{ curSongInfo.name }}</router-link>
                        <p><router-link :to="{ path: '/singer', query: { id: author.id } }" class="song_author"
                                v-for="(author, k) in curSongInfo.singer" :key="author.name">{{ k !== 0 ? ' / ' +
                                    author.name : author.name }}</router-link></p>
                    </div>
                    <div class="bar-time">
                        <span>{{ handleFormatSongTime(currentTime * 1000) }}</span> / {{ curSongInfo.duration }}
                    </div>
                </div>
                <div class="bar-m">
                ...
                <div class="bar-r">
                    <div class="bar-oper">
                        <div class="volume-main">
                            ...
                            <ProgressLine class="volumeLine" :progressWidth="volumeProgressWidth"
                                @setProgressLine="setvolumeProgress" />
                        </div>
                        <i class="iconfont" :class="modeIcon.className" :title="modeIcon.title"
                            ...
                            <el-popover placement="top" :width="550" trigger="click" @hide="popverClose">
                                ...
                                <div class="playlist-container">
                                    <h3 class="playlist-header">
                                        ...
                                        <div class="delSonglist" @click="clearSonglist"><i class="iconfont icon-del"
                                                title="清空列表"></i>清空列表</div>
                                    </h3>
...
import { computed, inject, nextTick, onBeforeUnmount, onMounted, reactive, ref, toRefs, watchEffect } from 'vue'
import { useStore } from 'vuex'
import { handleFormatSongSecond, handleFormatSongTime } from '@/common/util'
const store = useStore();

const emits =defineEmits(['playAudioMode','setVolumeHandler','setvolumeProgress','audioHandler','setAudioProgress','changeMini'])
const info = reactive({
    // 歌词弹窗时，固定播放条
    isLock: false,
    locked: false,
...
const changePlayMode = () => {
    ...
    emits('playAudioMode', info['playMode']);
};

// 音频播放进度条
const audioProgressWidth = computed(() => { // 音频进度条长度
    return info['currentTime'] / handleFormatSongSecond(curSongInfo.value.duration) * 100 + '%'
});
// 设置音频音量进度条
...
const volumeHandler = () => {
    ...
    emits('setVolumeHandler', info['isMuted']);
};
...
const setvolumeProgress = (params) => {
    ...
    emits('setvolumeProgress', params.val);
}
...
const setAudioProgress = (params) => {
    ...
    info['currentTime'] = params.val * handleFormatSongSecond(curSongInfo.value.duration);

    // 拖拽的时候，不实时更改音频的时间
    if (params.flag) {
        emits('setAudioProgress', info['currentTime']);
    }
};

const changeMini = () => {
    emits('changeMini', 'MiniBar');
}
