<template>
    <audio v-if="curSongInfo" ref="myAudio" preload="auto" @canplay="canplaySong" @playing="playSong" @ended="endedSong"
        @error="errorSong" @timeupdate="updateSongTime" :src="curSongInfo.url"></audio>
</template>
...
const updateSongTime = (e) => {
    ...
    emits('setCurrentTime', e.target.currentTime);
};
...
defineExpose({
    playAudioMode, playAudioType, setvolumeProgress,setVolumeHandler,setAudioProgress
})
</script>
<style scoped></style>