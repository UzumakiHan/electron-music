                    ...
                    <div class="cover-info">
                        <div class="cover-header">
                            <div class="cover-title">
                                {{ rankInfo.name }} <span>({{ handleFormatTime(rankInfo.updateTime, 'MM月DD日') }} 更新)</span>
                            </div>
                        </div>
                        <div class="cover-author-tags">
                            <div class="cover-author" v-if="rankInfo.creator">
                                ...
                                <div class="cover-date">{{ handleFormatTime(rankInfo.createTime, 'YYYY-MM-DD') }}</div>
                            </div>
                        </div>
                        <div class="cover-digital">
                            <span class="cover-playCount"><i class="iconfont icon-playnum"></i>
                                {{ handleFormartNum(rankInfo.playCount) }}次</span>
                            <span class="cover-collect"><i class="iconfont icon-collect"></i>
                                {{ handleFormartNum(rankInfo.subscribedCount) }}</span>
                            <span class="cover-comment"><i class="iconfont icon-comment"></i>
                                {{ handleFormartNum(rankInfo.commentCount) }}</span>
                        </div>
...
import { handleFormatTime, handleFormartNum,handleFormatSongs } from '@/common/util'
import { ajaxTopListDetail, ajaxListDetail } from '@/common/api'
...
const getTopListDetail = async () => {
    const listRes = await ajaxTopListDetail();
    if (listRes.code === 200) {
        info.listTop = listRes.list.filter(item => {
            return item.ToplistType
        })
        // 云音乐特色榜
        info['listFeature'] = listRes.list.filter(item => {
            return !item.ToplistType && item.name.indexOf('云音乐') >= 0
        })
        // 其他场景榜
        info['listOther'] = listRes.list.filter(item => {
            return !item.ToplistType && item.name.indexOf('云音乐') < 0
        })
...
const getListDetail = async () => {
    info['isLoading'] = true;
    const res = await ajaxListDetail({ id: info.rId, s: -1 })
    ...
    info['songList'] = handleFormatSongs(res.playlist.tracks, res.privileges)
