                ...
                <div class="song-l">
                    <div class="song-info">
                        <div class="info-top">
                            <h3 class="song-name">
                                {{ songInfo.name }}
                                <router-link class="mv-name" :to="{ path: '/mvlist/mv', query: { id: songInfo.mvId } }"
                                    v-if="songInfo.mvId">
                                    <i class="iconfont icon-mvlist"></i>
                                </router-link>
                                <i v-if="songInfo.vip" class="iconfont icon-vip"></i>
                            </h3>
                            <p><router-link :to="{ path: '/singer', query: { id: author.id } }" class="song-author"
                                    v-for="(author, k) in songInfo.singer" :key="author.name">{{ k !== 0 ? ' / ' +
                                        author.name : author.name }}</router-link></p>
                            <p class="song-related">
                                <span>专辑：<router-link class="song-album"
                                        :to="{ path: '/album', query: { id: songInfo.album.id } }">{{ songInfo.album.name
                                        }}</router-link></span>
                        ...
                        <div class="info-bottom">
                            <div class="song-oper">
                                <span :class="['play-btn', 'play-all', songDisable]" @click="plyaing(songInfo)"><i
                                        :class="['iconfont', playFontIcon]"></i> {{ songInfo.vip ? 'VIP尊享' : '立即播放' }}</span>
                                <span class="play-btn play-collect" @click="showAddList"><i
                                        class="iconfont icon-collect"></i> 收藏</span>
                                <span class="play-btn play-comment" @click="jumpComment"><i
                                        class="iconfont icon-comment"></i> 评论</span>
                            </div>
                        </div>
                    </div>
                    <div class="simi-song">
                        ...
                        <div class="simi-main">
                            <div class="simi-item" v-for="simiItem in simiSong" :key="simiItem.id">
                                <div class="simi-info">
                                    ...
                                    <div class="simi-author"><router-link :to="{ path: '/singer', query: { id: author.id } }"
                                            class="song-author" v-for="(author, k) in simiItem.singer" :key="author.name">{{
                                                k !== 0 ? ' / ' + author.name : author.name }}</router-link></div>
                                </div>
        ...
        <div class="aside-box">
            <div class="cover">
                ...
                <div class="cover-img" :class="[isCurSong ? 'active' : '']" @click.stop="plyaing(songInfo)">
                    <img :src="stylusIcon" class="cover-stylus" />
            ...
            <div class="sidebar-box playlist-simi" v-if="playlists.length">
                ...
                <div class="aside-main playlist-main">
                    <router-link class="playlist-item" :to="{ path: '/playlist/detail', query: { id: item.id } }"
...
import { ajaxSongDetail, ajaxSimiSong, ajaxSimiPlayList } from '@/common/api'
import { handleFormatSongs, handleFormatSongTime } from '@/common/util'
import stylusIcon from '@/assets/img/stylus.png'
...
const getSongDetail = async () => {
    const songDetailRes = await ajaxSongDetail({ ids: info.sId, timestamp: new Date().valueOf() })

    if (songDetailRes.code === 200) {
        // 是否有版权播放
        songDetailRes.songs[0].license = !songDetailRes.privileges[0].cp;
        info['songInfo'] = handleFormatSongs(songDetailRes.songs, songDetailRes.privileges)[0];
        // 显示歌曲简介
...
const getSimiSong = async () => {
    const simiSongRes = await ajaxSimiSong({ id: info.sId })
    console.log(simiSongRes)

    if (simiSongRes.code === 200) {
        info['simiSong'] = simiSongRes.songs.map(item => {
            return {
                id: String(item.id),
                name: item.name,
                mvId: item.mvid,
                ...
                album: item.album,
                alia: item.alias,
                duration: handleFormatSongTime(item.duration),
...
const getSimiPlayList = async () => {
    const playRes = await ajaxSimiPlayList({ id: info.sId })

    if (playRes.code === 200) {
        info['playlists'] = playRes.playlists;
    }
