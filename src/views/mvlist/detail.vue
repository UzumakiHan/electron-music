    ...
    <div v-if="videoOptions.src" class='mv-detail'>
        <div class="mv-main">
            <div class="mv-info-hd">
                ...
                <router-link :to="{ path: '/singer', query: { id: author.id } }" class="song-author"
                    v-for="(author, k) in mvDetail.artists" :key="author.name">{{ k !== 0 ? ' / ' + author.name :
        ...
        <div class="aside-box">
            <div class="mv-info">
                ...
                <div class="mv-info-count">
                    ...
                    <div class="info-count">播放量：{{ handleFormartNum(mvDetail.playCount) }}</div>
                </div>
                <div class="mv-desc">
            ...
            <div class="simi-mv">
                ...
                <div class="aside-main mv-main">
                    <div class="item" :key="'' + item.id + index" v-for="(item, index) in simiMv">
                        ...
                        <div class="info">
                            <router-link :to="{ path: '/mvlist/mv', query: { id: item.id } }"
                                class="mv-name">{{ item.name }}</router-link>
                            <router-link :to="{ path: '/singer', query: { id: item.artistId } }" class="mv-author"
                                v-if="!item.publishTime">{{ item.artistName }}</router-link>
                            <div class="mv-playCount"><i class="iconfont icon-mvlist"></i>
                                {{ handleFormartNum(item.playCount) }}</div>
...
import { ajaxMvDetail, ajaxMvUrl, ajaxCommentMv, ajaxSimiMv } from '@/common/api';
...
const getMvDetail = async () => {
    const mvDetailRes = await ajaxMvDetail({ id: info.mId })

    if (mvDetailRes.code === 200) {
        info.mvDetail = mvDetailRes.data;
        info.videoOptions.poster = mvDetailRes.data.cover;
    }
...
const getMvUrl = async (r) => {
    const mvUrlRes = await ajaxMvUrl({ id: info.mId, r })

    if (mvUrlRes.code === 200) {
        info.videoOptions.src = mvUrlRes.data.url;
    }
...
const getSimiMv = async () => {
    const simiMvRes = await ajaxSimiMv({ id: info.mId })

    if (simiMvRes.code === 200) {
        info.simiMv = simiMvRes.mvs
    }
