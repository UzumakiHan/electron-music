            ...
            <div class="detail-main">
                    ...
                    <div class="cover-info">
                        <div class="cover-header">
                            ...
                            <div class="cover-collect-author">
                                <router-link class="collect-author" :to="{ path: '/singer', query: { id: item.userId } }"
                                    v-for="item in collects" :key="item.userId">
                        ...
                        <div class="cover-author-tags">
                            <div class="cover-author" v-if="details.creator">
                                ...
                                <div class="cover-date">{{ handleFormatTime(details.createTime, 'YYYY-MM-DD') }}</div>
                            </div>
                            <div class="cover-tags" v-if="details.tags">
                                <router-link :to="{ path: '/playlist', query: { cat: tag } }" class="tag"
                                    v-for="(tag, index) in details.tags" :key="index">#{{ tag }}</router-link>
                            </div>
                        </div>
                        <div class="cover-digital">
                            <span class="cover-playCount"><i class="iconfont icon-playnum"></i>
                                {{ handleFormartNum(details.playCount) }}次</span>
                            <span class="cover-collect"><i class="iconfont icon-collect"></i>
                                {{ handleFormartNum(details.subscribedCount) }}</span>
                            <span class="cover-comment"><i class="iconfont icon-comment"></i>
                                {{ handleFormartNum(details.commentCount) }}</span>
                        </div>
                ...
                <div class="song-main">
                    <div class="song-header">
                        ...
                        <span :class="['collect', details.subscribed ? 'active' : '']" @click="subPlayList(details)"><i
                                :class="['iconfont', 'icon-collect' + (details.subscribed ? '-active' : '')]"></i> {{
            ...
            <div class="aside-box">
                <div class="playlist-recom" v-if="playlists.length">
                    ...
                    <div class="aside-main recom-main">
                        <router-link class="recom-item" :to="{ path: '/playlist/detail', query: { id: item.id } }"
                    ...
                    <div class="aside-main comment-main">
                        <div class="comment-item" v-for="item in comments" :key="item.commentId">
                            ...
                            <div class="comment-info">
                                <div class="comment-userInfo"><router-link
                                        :to="{ path: '/user', query: { id: item.commentId } }"
                                        class="comment-name">{{ item.user.nickname }}</router-link><span
                                        class="comment-date">{{ handleFormatTime(item.time,'YYYY-MM-DD') }}</span></div>
...
import { ajaxPlaylistdetail, ajaxSongDetail, ajaxPlaylistsCollect, ajaxPlaylistRelated, ajaxPlaylistComment, ajaxSubPlayList } from '@/common/api'
import { handleFormatSongs ,handleFormartNum,handleFormatTime,handleFormatMsgTime} from '@/common/util'
...
const { id, details, isShowDesc, songList, collects, playlists, comments, total, isLoading } = toRefs(info)
...
const getDetail = async (params) => {
    info['isLoading'] = true;
    const detailRes = await ajaxPlaylistdetail(params)

    if (detailRes.code === 200) {
        info['details'] = detailRes.playlist;

        if (isLogin.value) {
            const ids = detailRes.playlist.trackIds;

            getAllSongs(ids);
        } else {
            info['songList'] = handleFormatSongs(detailRes.playlist.tracks, detailRes.privileges);
...
const getAllSongs = async (ids) => {
    ...
    for (let i = 0; i < sliceArr.length; i++) {
        ...
        info['isLoading'] = true;
       const detailRes = await ajaxSongDetail({ ids: arrs.join(','), timestamp: new Date().valueOf() + i })

        idsArr = idsArr.concat(handleFormatSongs(detailRes.songs, detailRes.privileges))
    }
...
const getCollect = async (params) => {
    ...
    const collectRes = await ajaxPlaylistsCollect(params)

    if (collectRes.code === 200) {
        info.collects = collectRes.subscribers
    }
...
const getRecom = async (params) => {
    const relateRes = await ajaxPlaylistRelated(params)

    if (relateRes.code === 200) {
        info.playlists = relateRes.playlists
    }
...
const getComment = async (params) => {
    const commentRes = await ajaxPlaylistComment(params)

    if (commentRes.code === 200) {
        info.comments = commentRes.comments
    }
...
const subPlayList = async (item) => {
    const playRes = await ajaxSubPlayList({ id: item.id, t: (item.subscribed ? 2 : 1) })
