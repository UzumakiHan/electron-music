    ...
    <div class='artist'>
        <div class="artist-container">
            <div class="artist-main">
                <div class="list-container" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy"
                    ...
                    <div class="aside-menu">
                        <div class="filter">
                            <div class="filter-item">
                                <span v-for="(item, index) in initial" :key="index"
                                    :class="index === initialIndex ? 'active' : ''"
                                    @click="selectType('initial', index)">{{ item.label }}</span>
                            </div>
                            <div class="filter-item">
                                <span v-for="(item, index) in area" :key="index"
                                    :class="index === areaIndex ? 'active' : ''"
                                    @click="selectType('area', index)">{{ item.label }}</span>
                            </div>
                            <div class="filter-item">
                                <span v-for="(item, index) in type" :key="index"
                                    :class="index === typeIndex ? 'active' : ''"
                                    @click="selectType('type', index)">{{ item.label }}</span>
                            </div>
                        </div>
                    </div>
                </el-affix>
...
import { ajaxArtistList } from '@/common/api'
const info = reactive({
...
const { type, area, initial, typeIndex, areaIndex, initialIndex, params, list, isLoading, busy } = toRefs(info)
...
const getArtist = async (params) => {
    const artistRes = await ajaxArtistList(params)

    if (artistRes.code === 200) {
        info.list = info.params.offset !== 0 ? [...info.list, ...artistRes.artists] : artistRes.artists
        info.busy = !artistRes.more
        info.isLoading = artistRes.more
    }
