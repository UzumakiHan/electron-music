            ...
            <div class="detail-main">
                <div class="album-cover">
                    ...
                    <div class="album-info">
                        ...
                        <div class="album-time">发行时间：{{handleFormatTime(details.publishTime, 'YYYY-MM-DD')}}</div>
                        ...
                        <router-link class="aside-album-item" :to="{ path: '/album', query: { id: item.id }}" v-for="item in hotAlbums" :key="item.id">
                            ...
                            <div class="aside-album-info">
                                ...
                                <div class="aside-album-time">
                                    {{handleFormatTime(details.publishTime, 'yyyy-MM-dd')}}
                                </div>
                            </div>
                        </router-link>
                    </div>
...
import{ajaxAlbum,ajaxAlbumDynamic,ajaxArtistAlbum,ajaxAlbumSub,ajaxPlaylistsCollect} from '@/common/api'
import {handleFormatSongs,handleFormatTime} from '@/common/util'
...
const {albumId,details,songList,dynamic,hotAlbums,comments,type,isShowDesc,collects} = toRefs(info)
        // 相关歌单推荐
        const getAlbum = async(params) => {
            const res = await ajaxAlbum(params)
            ...
            info.songList = handleFormatSongs(res.songs, privileges);
            getArtistAlbum();
            }
        ...
        const getAlbumDynamic = async(params) => {
          const  res = await ajaxAlbumDynamic(params)
        ...
        const getArtistAlbum = async() => {
          const res = await ajaxArtistAlbum({ id: info.details.artists[0].id, limit: 5 })
        ...
        const subAlbum = async() => {
            const res=await ajaxAlbumSub({ id: info.albumId, t: Number(!info.dynamic.isSub) })
        ...
        const getCollect = async(params) => {
            const res= await ajaxPlaylistsCollect(params)
